import http from './public'
// 商品列表
export const getAllGoods = (params) => {
  return http.fetchGet('/item/list', params)
}
// 商品列表秒杀
export const getPromoItems = (params) => {
  return http.fetchGet('/item/promoItems', params)
}
// 获取购物车列表
export const getCartList = (params) => {
  return http.fetchPost('/cart/getList', params)
}
// 加入购物车
export const addCart = (params) => {
  return http.fetchPost('/cart/addCart', params)
}
// 删除购物车 服务端没有************
export const delCart = (params) => {
  return http.fetchPost('/member/delCart', params)
}
// 删除购物车勾选商品 服务端没有************
export const delCartChecked = (params) => {
  return http.fetchPost('/cart/delCartChecked', params)
}
// 编辑购物车 服务端没有************
export const cartEdit = (params) => {
  return http.fetchPost('/cart/cartEdit', params)
}
// 全选
export const editCheckAll = (params) => {
  return http.fetchPost('/cart/editCheckAll', params)
}
// 删除整条购物车
export const cartDel = (params) => {
  return http.fetchPost('/cart/cartDel', params)
}
// 获取用户地址
export const addressList = (params) => {
  return http.fetchPost('/address/getList', params)
}
// 通过id获取地址
export const getAddress = (params) => {
  return http.fetchPost('/address/address', params)
}
// 修改收货地址
export const addressUpdate = (params) => {
  return http.fetchPost('/address/updateAddress', params)
}
// 添加收货地址
export const addressAdd = (params) => {
  return http.fetchPost('/address/addAddress', params)
}
// 删除收货地址
export const addressDel = (params) => {
  return http.fetchPost('/address/delAddress', params)
}
// 生成订单
export const submitOrder = (params) => {
  return http.fetchPost('/order/create', params)
}
// 生成秒杀令牌，校验验证码
export const generateToken = (params) => {
  return http.fetchGet('/order/generateToken', params)
}
// 支付 服务端没有************
export const payMent = (params) => {
  return http.fetchPost('/order/payOrder', params)
}
// 获取用户订单
export const orderList = (params) => {
  return http.fetchGet('/order/getList', params)
}
// 获取单个订单详情
export const getOrderDet = (params) => {
  return http.fetchGet('/order/orderDetail', params)
}
// 取消订单 服务端没有************
export const cancelOrder = (params) => {
  return http.fetchPost('/member/cancelOrder', params)
}
// 商品详情
export const productDet = (params) => {
  return http.fetchGet('/item/get', params)
}
// 删除订单
export const delOrder = (params) => {
  return http.fetchGet('/order/delOrder', params)
}
// 商品列表
export const getSearch = (params) => {
  return http.fetchGet('/item/list', params)
}
// 快速搜索 服务端没有************
export const getQuickSearch = (params) => {
  return http.fetchGet('/goods/quickSearch', params)
}
