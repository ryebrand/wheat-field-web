...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 修改购物车
      _cartEdit (userId, itemId, amount, checked) {
        cartEdit(
          {
            userId,
            itemId,
      ...
      editCart (type, item) {
        if (type && item) {
          ...
          if (type === 'check') {
            ...
            this.$set(item, 'checked', newChecked)
            // this._cartEdit(this.userId, itemId, amount, newChecked)
          }
        } else {
          console.log('缺少所需参数')
        }
      },
      EditNum (amount, itemId, checked) { // 数量
        this._cartEdit(this.userId, itemId, amount, checked)
      },
      // 删除整条购物车
      cartdel (itemId) {
      ...
      checkout () {
        window.sessionStorage.setItem('cartActive', JSON.stringify(this.cartList))
...
<style lang="scss" rel="stylesheet/scss" scoped>
  ...
  .cart-e {
    margin: 0 auto;
    background: url("/resources/static/images/cart-empty_@2x.png") no-repeat;
    width: 275px;
    height: 300px;
    color: #8d8d8d;
  }
