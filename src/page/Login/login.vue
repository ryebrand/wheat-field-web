        ...
        <div v-if="loginPage" class="content">
          <ul class="common-form">
            <li class="username border-1p">
              <div class="input">
                <input type="text" v-model="ruleForm.userName" placeholder="账号/手机号">
              </div>
            </li>
            <li>
              <div class="input">
            ...
            <li style="text-align: right" class="pr">
              ...
              <a href="javascript:;" class="register" @click="toRegister">注册 麦田 账号</a>
          ...
          <div class="footer">
            ...
            <a><img @click="open('待开发','此功能开发中...')" style="height: 15px; margin-top: 22px;" src="/resources/static/images/other-login.png"></a>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getRemembered () {
      ...
      if (judge === 'true') {
        this.autoLogin = true
        this.ruleForm.userName = getStore('telephone')
    ...
    rememberPass () {
      if (this.autoLogin === true) {
        ...
        setStore('telephone', this.ruleForm.userName)
        setStore('password', this.ruleForm.userPwd)
      } else {
        setStore('remember', 'false')
        removeStore('telephone')
    ...
    login () {
      ...
      userLogin(params).then(res => {
        if (res.status === 'success') {
          ...
          checkToken.saveToken(res.data.token)
          // 登录后添加当前缓存中的购物车
          if (this.cart.length) {
  ...
  mounted () {
    ...
    this.login_addCart()
    // this.init_geetest()
    this.open('登录提示', '测试体验账号密码：110 | jack')
  },
  components: {
    YFooter,
    YButton
