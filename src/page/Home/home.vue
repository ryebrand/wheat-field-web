<template>
  ...
  <div v-loading="loading" element-loading-text="加载中..." style="min-height: 35vw;" v-if="!error">
    <div class="banner" >
      <div class="bg" ref="bg"
        ...
        <transition name="fade">
          <div v-for="(item, i) in banner" v-if="i===mark" :key="i" style="position:absolute" @click="openProduct(item.itemId)" @mouseover="stopTimer" @mouseout="startTimer">
            <img v-if="item.homeScroll" class="img1" :src="item.homeScroll"/>
            // @click="linkTo(item)"
        ...
        <y-shelf :title="item.name">
          <div slot="content" class="floors" >
            ...
            <mall-goods :msg="iitem" v-for="(iitem,j) in item.panelContents" :key="j+'key'"
                        v-if="iitem.type != 2 && iitem.type != 3" isShowBtn="promo"></mall-goods>
          </div>
        </y-shelf>
      </section>
    ...
    <div class="no-info" v-if="error">
      <div class="no-data">
        <img src="/resources/static/images/error.png">
        <br> 抱歉！出错了...
      </div>
    </div>
