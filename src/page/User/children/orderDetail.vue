    ...
    <y-shelf v-bind:title="orderTitle">
      <div slot="content">
        <div v-loading="loading" element-loading-text="加载中..." style="min-height: 10vw;" v-if="orderList.length">
          <div class="orderStatus" v-if="status !== -1 && status !== 6">
          ...
          <div class="orderStatus-close" v-if="status === -1">
            <el-steps :space="780" :active="2">
              <el-step title="下单" v-bind:description="createTime"></el-step>
              <el-step title="交易关闭" v-bind:description="finishData"></el-step>
            </el-steps>
          </div>
          <div class="orderStatus-close" v-if="status === 6">
            <el-steps :space="780" :active="2">
              <el-step title="下单" v-bind:description="createTime"></el-step>
              <el-step title="交易关闭" v-bind:description="finishData"></el-step>
            </el-steps>
          ...
          <div class="status-now" v-if="status === 1">
            <ul>
              ...
              <li class="button">
                <el-button @click="orderPayment(id)" type="primary" size="small">现在付款</el-button>
          ...
          <div class="goods-table">
            <div class="cart-items" v-for="(item,i) in orderList" :key="i">
              <a @click="goodsDetails(item.itemId)" class="img-box"><img :src="item.imgUrl" alt=""></a>
              <div class="name-cell ellipsis">
                <a @click="goodsDetails(item.itemId)" title="" target="_blank">{{item.title}}</a>
              </div>
              <div class="n-b">
                <div class="price">¥ {{Number(item.price).toFixed(2)}}</div>
                <div class="goods-num">{{item.amount}}</div>
                <div class="subtotal"> ¥ {{Number(item.price * item.amount).toFixed(2)}}</div>
              </div>
            </div>
          </div>
          <!--合计-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      orderPayment (id) {
        window.open(window.location.origin + '#/order/payment?orderId=' + id)
      },
      goodsDetails (itemId) {
        this.$router.push({name: 'goodsDetails', query: {id: itemId}})
        // window.open(window.location.origin + '#/goodsDetails?id=' + itemId)
      },
      _getOrderDet () {
        let params = {
          params: {
        ...
        getOrderDet(params).then(res => {
          ...
          this.orderTotal = res.data.totalPrice
          this.userName = res.data.address.userName
          this.tel = res.data.address.telephone
          this.streetName = res.data.address.address
          this.createTime = res.data.createDate
          this.finishData = res.data.finishDate
          this.payTime = res.data.finishDate
          if (this.status === 5) {
            this.finishTime = res.data.finishDate
          } else {
            this.countTime = res.data.finishDate
          }
          this.loading = false
        })
      },
